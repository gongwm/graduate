...
function changeTheme(type) {
	var themePath = "/HPPumTurReg/js/easyui-1.4.5/themes/{0}/easyui.css";
	$("#theme").attr("href", themePath.format(type));
}

function changeToParentTheme() {
	var theme = _rootWindow().current_theme;
	if (!_.isNil(theme)) {
		changeTheme(theme);
	}
}
...
/**
 * 删除easyui datagrid的若干行数据，当其列columnName等于value的时候。删除一行。注意使用值唯一的列。
 * 
 * @param tableSelector
 *            table选择器
 * @param columnName
...
function buttonDisabled(id) {
	return S(id).linkbutton("options").disabled;
}
 ...
 *            textbox选择器
 */
function clearText(/* selector,prefix */) {
	var selector = ".easyui-textbox";
	if (arguments.length == 1) {
		...
		if (_.isString(first)) {
			...
			var ids = first;
			for ( var idx in ids) {
				S(ids[idx]).textbox("setValue", "");
			}
			return;
		}
	} else if (arguments.length == 2) {
		selector = "{0}[id^={1}]".format(arguments[0], arguments[1]);
	}
	$(selector).each(function() {
 ...
 * @param win
 */
function arrangeLocation(win) {
	var w = $(window).width();
	var h = $(window).height();
	var options = win.window("options");
	var mwidth = options.width, mheight = options.height;
	var top = (h - mheight) / 2;
	top = top < 0 ? 10 : top;
	var left = (w - mwidth) / 2;
	win.window("move", {
...
$(function() {
	...
	var html = '<div id="'
			...
			<tr><td><img alt="waiting..." src="/HPPumTurReg/images/waiting.gif" \
...
function showMask() {
	if (!mask_open) {
		...
		$(mask_id_selector).window("center");
		mask_open = true;
	}
}
...
$(function() {
	...
	var html = '<div id="{0}" class="easyui-window" title="打印预览" \
			...
			<iframe id="{1}" name="{2}"	style="width: 100%; height: 100%; \
		    border: none"></iframe></div>'
			.format(previewer_div, previewer_id, previewer_name);
	$("body").append(html);
...
function previewPage(url, postData) {
	...
	$.post(url, postData, function(data) {
		if (_.isNil(data)) {
			...
			window.frames[previewer_name].location.href = data.htmlPath;
			var win = $(previewer_div_selector);
			win.window('open');
			arrangeLocation(win);
		}
		hideMask();
...
function showWindow(title, hiddenId, size) {
	var win = $(previewer_div_selector);
	var options = win.window("options");
	options.title = title;
	options.content = S(hiddenId).clone().show();
	if (!_.isNil(size)) {
		options.width = size.width || 800;
		options.height = size.height || 450;
	}
	win.window(options);
	win.window("open");
	arrangeLocation(win);
...
function showWindowContent(title, content) {
	var win = $(previewer_div_selector);
	var options = win.window("options");
	options.title = title;
...
function hideWindow() {
	$(previewer_div_selector).window("close");
}
