...
def compress(files, skip=['//', '/*', '*/', '*'],
    ...
    with open(outfile, 'w', encoding='utf-8') as out:
        ...
        [print('compressing {}'.format(f)) for f in filtered_files]
        for file in filtered_files:
            p = Path(file)
            if p.is_file() and p.suffix == '.js':
                with open(file, 'r', encoding='utf-8') as f:
                    out.write(''.join([exclude_inline_comment(line)
                                       for line in f if should_include(line)]))
            elif p.is_dir():
...
if __name__ == '__main__':
    ...
    print('input name for out file: ')
    outfile = input()

    compress(files, outfile=outfile)
